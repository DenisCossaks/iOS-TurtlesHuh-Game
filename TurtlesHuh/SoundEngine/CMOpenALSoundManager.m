...
void interruptionListenerCallback2 (void *inUserData, UInt32 interruptionState) 
{
...
- (id) init
{
	...
	{		
		...
		OSStatus status = AudioSessionInitialize(NULL, NULL, interruptionListenerCallback2, self);
...
- (void) playBackgroundMusic:(NSString *)file isLoop:(BOOL)_isLoop
{
	//ORIGINAL IT USE NO
	//[self playBackgroundMusic:file forcePlay:NO];
	[backgroundAudio stop];
	[self playBackgroundMusic:file forcePlay:NO isLoop:(BOOL)_isLoop];
}

- (void) playBackgroundMusic:(NSString *)file forcePlay:(BOOL)forcePlay isLoop:(BOOL)_isLoop
{		
...
-(ALuint) getSourceID:(NSUInteger)soundID{
...
- (void) playSoundWithID:(NSUInteger)soundID isLoop:(BOOL)_isLoop withPitch:(float)_pitch gain:(float)_gain
{	
	//get sound key
	NSString *soundFile = [self keyForSoundID:soundID];
	if(!soundFile) return;
	...
	{
		//create a new sound
		sound = [[CMOpenALSound alloc] initWithSoundFile:soundFile doesLoop:_isLoop withPitch:_pitch]; //this will return nil on failure
	...
	//sound.volume = _gain*0.5;
    sound.volume = _gain*1.0;
}
